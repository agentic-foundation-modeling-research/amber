# Append Memory Training

End-to-end training for the append memory format: first SFT bootstrap, then GRPO RL
...
[packages/train/training_configs/sft](../../packages/train/training_configs/sft) — append
memory uses [append_memory.yaml](../../packages/train/training_configs/sft/append_memory.yaml),
...
CHECKPOINT_NAME=qwen3.5-append-memory-sft

# Append memory training data
export MILES_SCRIPT_PROMPT_DATA=$STORAGE_ROOT/datasets/webarena_append_memory_sft/data/train-00000-of-00001.parquet

# Rollout function and training hyperparameters
export MILES_SCRIPT_CUSTOM_CONFIG_PATH="packages/train/training_configs/sft/append_memory.yaml"
...
### Training Config
The rollout config can be changed through custom YAML files. Append memory rollouts use
[append_memory.yaml](../../packages/train/training_configs/rl/append_memory.yaml).
...
STORAGE_ROOT=/mnt/shared-storage # Your shared filesystem mount
CHECKPOINT_NAME=qwen3.5-append-memory-sft+rl # Replace
export MILES_SCRIPT_OUTPUT_DIR="$STORAGE_ROOT/workspace/${RL_USER}/${CHECKPOINT_NAME}"
export MILES_SCRIPT_HF_CHECKPOINT="$STORAGE_ROOT/workspace/${RL_USER}/hf_checkpoints/qwen3.5-append-memory-sft/" # Model trained using SFT
export MILES_SCRIPT_MEGATRON_CHECKPOINT="$STORAGE_ROOT/workspace/${RL_USER}/torch_dist_checkpoints/qwen3.5-append-memory-sft/" # Converted from HF

# Config that governs the rollout code
export MILES_SCRIPT_CUSTOM_CONFIG_PATH="packages/train/training_configs/rl/append_memory.yaml"
...
Evaluation runs through [scripts/eval/append.sh](../../scripts/eval/append.sh), which
pins the flags for this method (notably `--memory_format append`). All of the
