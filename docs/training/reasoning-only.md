# Reasoning Only Training

End-to-end training for the reasoning-only format (the WebAgentR1 baseline): first SFT
bootstrap, then GRPO RL starting from the SFT checkpoint. There is no memory turn — each
step is a single action turn whose reasoning is the only state carried forward — so both
the SFT rollout function and the RL rollout code path differ from the memory methods
(`sft_generate.generate_bc` and `generate_reasoning` instead of
`sft_generate.generate_sft` and `generate_webarena`).
...
reasoning only uses
[reasoning_only.yaml](../../packages/train/training_configs/sft/reasoning_only.yaml),
which sets the rollout function (`sft_generate.generate_bc`, behavior cloning on the
action turn alone) along with `num_epoch`, `rollout_batch_size`, `global_batch_size`,
`max_tokens_per_gpu`, `save_interval`, and `lr`. Edit that file to change hyperparameters.
...
The SFT data is the behavior cloning dataset (`webarena_reasoning_only_sft`), whose prompts
have three messages (system, user, assistant action) instead of the five-message memory
format; see [data_preparation.md](../data_preparation.md).
...
CHECKPOINT_NAME=qwen3.5-reasoning-only-sft

# Reasoning only (behavior cloning) training data
export MILES_SCRIPT_PROMPT_DATA=$STORAGE_ROOT/datasets/webarena_reasoning_only_sft/data/train-00000-of-00001.parquet

# Rollout function and training hyperparameters
export MILES_SCRIPT_CUSTOM_CONFIG_PATH="packages/train/training_configs/sft/reasoning_only.yaml"
...
### Training Config
The rollout config can be changed through custom YAML files. Reasoning-only rollouts use
[reasoning_only.yaml](../../packages/train/training_configs/rl/reasoning_only.yaml), which
points the rollout, generate, reward model, and reward post-process hooks at
`generate_reasoning` and omits the memory keys (`webarena_memory_format`,
`webarena_debug_loss_mask`) that the memory configs carry.
...
STORAGE_ROOT=/mnt/shared-storage # Your shared filesystem mount
CHECKPOINT_NAME=qwen3.5-reasoning-only-sft+rl # Replace
export MILES_SCRIPT_OUTPUT_DIR="$STORAGE_ROOT/workspace/${RL_USER}/${CHECKPOINT_NAME}"
export MILES_SCRIPT_HF_CHECKPOINT="$STORAGE_ROOT/workspace/${RL_USER}/hf_checkpoints/qwen3.5-reasoning-only-sft/" # Model trained using SFT
export MILES_SCRIPT_MEGATRON_CHECKPOINT="$STORAGE_ROOT/workspace/${RL_USER}/torch_dist_checkpoints/qwen3.5-reasoning-only-sft/" # Converted from HF

# Config that governs the rollout code
export MILES_SCRIPT_CUSTOM_CONFIG_PATH="packages/train/training_configs/rl/reasoning_only.yaml"
...
[scripts/eval/reasoning_only.sh](../../scripts/eval/reasoning_only.sh), which uses the
reasoning-only entrypoint (`scripts/eval/eval_webarena_reasoning_only.py`) and takes no
`--memory_format` flag. The decode budget is capped at `--llm_max_tokens 1024`, since the
model only emits the action turn. All of the environment-specific values are read from
exported variables, so the script itself never needs editing.
...
export MODEL=qwen3.5-reasoning-only-sft+rl

# Output goes to outputs/evals_30_steps/$RUN_NAME, so keep RUN_NAME unique per run.
export RUN_NAME=qwen3.5-reasoning-only-sft+rl-seed100
export SEED=100

bash scripts/eval/reasoning_only.sh
```
