...
DEFAULT_MEMORY_LLM_MAX_TOKENS = 1024 + 2048
...
def parse_args(argv: list[str] | None = None) -> RunnerConfig:
    ...
    parser.add_argument(
        ...
        help="Maximum completion tokens for each action LLM call.",
    )
    parser.add_argument(
        "--memory_max_tokens",
        type=int,
        default=DEFAULT_MEMORY_LLM_MAX_TOKENS,
        help="Maximum completion tokens for each memory LLM call.",
    )
    parser.add_argument(
        "--llm_temperature",
        type=float,
    ...
    parser.add_argument(
        ...
        choices=("state",),
        default="state",
        help="Memory schema to use. Only state memory is supported.",
    )
    parser.add_argument(
        "--filter_viewport",
        action="store_true",
...
def call_llm_and_parse_memory_sync(
    llm: BaseLLM, messages: list,
    max_attempts: int,
    parser_cls: type[Memory],
    ...
    for attempt in range(1, max_attempts + 1):
        raw_response, misc = llm(messages, max_tokens=max_tokens)

        # Append the <think> tag as the server prefills it for us
        raw_response = think_prefix + raw_response

        try:
            return parser_cls.parse_response(raw_response), misc
        except (ReasoningParseError, MemoryParseError, StateParseError):
            logging.info(
                "Query failed. Retrying %s/%s.\n[LLM]:\n%s",
                attempt,
                max_attempts,
...
async def call_llm_and_parse_memory(
    llm: BaseLLM,
    messages: list,
    max_attempts: int,
    parser_cls: type[Memory],
    ...
    return await asyncio.to_thread(
        call_llm_and_parse_memory_sync,
        llm,
        messages,
        max_attempts,
...
async def agent_loop(
    ...
    **obs_kwargs,
    ...
    prompt_builder_obj = SingleTurnWithStateMemoryPromptBuilder()
    ...
    for step_num in range(max_steps):
        ...
        memory, _ = await call_llm_and_parse_memory(
            llm,
            messages,
            max_attempts=parse_retries,
            parser_cls=Memory,
        ...
        # print(memory.model_full_response)
...
def save_trajectory(
    ...
    task_id: int,
    rollout_index: int,
    rollout_trajectory: TrajectoryDataWithStateMemory,
) -> Path:
...
async def main_async(argv: list[str] | None = None) -> None:
    ...
    logging.info(
        "Using model: %s (provider=%s, base_url=%s, action_max_tokens=%s, memory_max_tokens=%s)",
        config.model,
        config.llm_provider,
