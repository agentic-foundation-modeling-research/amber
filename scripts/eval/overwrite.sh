#!/usr/bin/env bash
# Evaluate an overwrite memory checkpoint (MEM1 baseline) on WebArena Lite.
#
# Export the required variables before running; see docs/training/overwrite.md:
...
: "${ENV_SERVER_SERVICE_URL:?export it first, see docs/training/overwrite.md}"
: "${WEBSITES_VM_HOST:?export it first, see docs/training/overwrite.md}"
: "${LLM_BASE_URL:?export it first, see docs/training/overwrite.md}"
: "${MODEL:?export it first, see docs/training/overwrite.md}"
: "${RUN_NAME:?export it first, see docs/training/overwrite.md}"
: "${SEED:?export it first, see docs/training/overwrite.md}"
...
echo "==> overwrite memory | seed $SEED | $save_dir"
